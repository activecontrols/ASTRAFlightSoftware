...
py::tuple solve() {
    ...
    auto x = py::array_t<OSQPFloat>({n}, {sizeof(OSQPFloat)}, (&solver)->solution->x);
    auto y = py::array_t<OSQPFloat>({m}, {sizeof(OSQPFloat)}, (&solver)->solution->y);
...
OSQPInt update_data_mat(py::object P_x, py::object P_i, py::object A_x, py::object A_i) {
    ...
    return osqp_update_data_mat(&solver, _P_x, _P_i, _P_n, _A_x, _A_i, _A_n);
}
#endif

PYBIND11_MODULE(emosqp, m) {
    m.def("solve", &solve);
